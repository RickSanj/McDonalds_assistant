...
class ChildrenItem(BaseModel):
    ...
    type: str = Field(
        ...,
        description="Type of the item from the menu. Either 'fries' or 'drinks'."
    )
...
class OrderItem(BaseModel):
    ...
    size: Optional[str] = Field(  # Possible values: 'small', 'medium', 'large'.")
        None,
        description="Size of the item if applicable. See the menu."
    )
    ...
    children: Optional[List["ChildrenItem"]] = Field(
        None,
        description="Optional nested items for combos or double deals. For example, a combo contains burger, drink, and fries."
    )
